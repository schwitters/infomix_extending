...
memalign(size_t align, size_t size)
{
    ...
    if (!(newptr = (void *) malloc_func (size + sizeof (memheader) + align)))
	return NULL;
	
    ret = ALIGN (align, (char *)newptr + sizeof (memheader));
    mh = (memheader *)ret - 1;
    mh->mh_size = size;
    mh->mh_ptr = newptr;
...
{
        DPRINTF( "__myErrors__", 20, (".......in local new operator"));
        return(mi_alloc((mi_integer)size));
}
 
void
operator delete(void *pntr)
{
        DPRINTF( "__myErrors__", 20, (".......in local delete operator"));
        mi_free(pntr);
}
 
#ifdef CPLUSPLUS_HAS_ARRAY_NEW
void *
operator new[](size_t size)
{
        DPRINTF( "__myErrors__", 20, (".......in local new[] operator"));
