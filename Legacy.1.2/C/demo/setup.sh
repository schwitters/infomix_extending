#!/bin/sh
#
# setup.sh: Create a sample database and register idn_LegacyC.1.2
#
rm -f setup.out
echo " " > setup.out
...
create database idn_LegacyCdb with log;
END_CREATEDB

echo "	Step 3: Register the idn_LegacyC.1.2 DataBlade" >> setup.out
echo " " >> setup.out
blademgr >> setup.out 2>&1 <<END_BLADEMGR
register idn_LegacyC.1.2 idn_LegacyCdb
y
quit
END_BLADEMGR
