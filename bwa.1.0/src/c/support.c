...
**
**	Alternately, the TraceSet_BWA procedure may be
...
**	the following SQL:
**
**		CREATE PROCEDURE TraceSet_BWA
**		(
**			lvarchar,
**			int
**		)
**		EXTERNAL NAME "/path/.../BWA.bld(TraceSet_BWA)"
**		LANGUAGE C
**		END PROCEDURE;
**
**	Once registered, call this function by using the following SQL:
**
**		EXECUTE PROCEDURE TraceSet_BWA( "/mypath/myfile", 20 );
**
...
**
**		EXECUTE PROCEDURE TraceSet_BWA( "", 10 );
**
...
**
**	Warning: Do not remove or modify this comment:
**	    TraceSet_BWA FunctionId: 11190130-cf56-11d1-9ce6-080040e6b366
**
********************************************************************************
*/
...
)
{
	...
	*/
	if( Gen_TraceLevel >= 0 )
	{
		sprintf( Gen_TraceStr, "BWA %d", Gen_TraceLevel );
		...
		/* Also turn on __myErrors__ so that any errors
		** will also be output to the tracefile.
		** DBDK did not generate this call; it is a personal
		** style choice.
		*/
	...
	/* Finally, the DBDK_TRACE_ENTER and DBDK_TRACE_EXIT
	** macros call GL_DPRINTF. The first call to GL_DPRINTF
	** initializes the list of trace messages for the client
	** session by doing a select from the systracemsgs catalog.
	** So that first call to GL_DPRINTF is not parallelizable.
	**
	** We added a DBDK_TRACE_EXIT call below to also initialize
	** the trace messages list so the rest of the GL_DPRINTF calls
	** will be parallelizable.
	*/

	DBDK_TRACE_EXIT( "TraceSet_BWA" );
}
/* }}FUNCTION (#QV8H) */
