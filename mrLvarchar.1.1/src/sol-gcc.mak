# This Makefile builds the mrLvarchar DataBlade.

# TARGET must be set to the location/filename
# of the platform-specific make include file.
include $(TARGET)

# override some of the patform-specific variables for gcc
CC=gcc
CC_PIC=
...
PROJECTC_OBJS = \
	$(BINDIR)/util.$(OBJSUFF)     \
	$(BINDIR)/udr.$(OBJSUFF)      \
	$(BINDIR)/strfuncs.$(OBJSUFF) \
	$(BINDIR)/re_funcs.$(OBJSUFF) \
	$(BINDIR)/regerror.$(OBJSUFF) \
	$(BINDIR)/regexp.$(OBJSUFF)   \
	$(BINDIR)/regsub.$(OBJSUFF)   \
	$(BINDIR)/mrLvarchar.$(OBJSUFF)

PROJECT_LIBS = $(BINDIR)/$(PROJECT_TITLE).$(BLDLIB_SUFF)

all : $(BINDIR)
	$(MAKE) $(MAKEFLAGS) -f sol-gcc.mak server $(BUILD_TARGET)

# Construct each object file.

$(BINDIR)/util.$(OBJSUFF) : c/util.c
	$(CC) $(CFLAGS) -o $@ -c $?

$(BINDIR)/udr.$(OBJSUFF) : c/udr.c
	$(CC) $(CFLAGS) -o $@ -c $?

$(BINDIR)/strfuncs.$(OBJSUFF) : c/strfuncs.c
	$(CC) $(CFLAGS) -o $@ -c $?

$(BINDIR)/mrLvarchar.$(OBJSUFF) : c/mrLvarchar.c
	$(CC) $(CFLAGS) -o $@ -c $?

$(BINDIR)/re_funcs.$(OBJSUFF) : c/re_funcs.c
	$(CC) $(CFLAGS) -o $@ -c $?
...
c/strfuncs.c :  c/$(PROJECT_TITLE).h

c/mrLvarchar.c :  c/$(PROJECT_TITLE).h
