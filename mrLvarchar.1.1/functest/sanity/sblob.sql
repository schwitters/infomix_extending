...
-- Filename        : sblob.sql
-- Description     : Base i/o tests for large values.
-- 
--   Tests:
--
--      1. Input / Output 
...
--     17. Union query
--
--   Additional trace output is to /tmp/mrLvarchar_large.trc.
-- ---------------------------------------------------------------

--
-- Turn tracing on.
--
execute procedure TraceSet_mrLvarchar ("/tmp/mrLvarchar_large.trc", 20);
...
-- non-sblobs. We expect it to return 0 for the smart blob created
-- during Input. (We don't use lohandles() for this smart blob test
-- because it will generate a different handle for each run, messing
...
-- The new refcount should be 0 because the new object 
...
-- rename the explain output file to 'sqexplain.sblob.log'
!mv -f sqexplain.out sqexplain.sblob.log
