-- --------------------------------------------------------------------------
-- 
-- Module          : Functional Test for instr(idn_mrLvarchar)
-- Filename        : instr.sql
...
--
-- If enabled, additional trace info gets output to /tmp/mrLvarchar_instr.trc.
-- --------------------------------------------------------------------------

--
-- Turn tracing on.
--
-- execute procedure TraceSet_mrLvarchar ("/tmp/mrLvarchar_instr.trc", 20);
...
select mrcol, instr(mrcol, ' ') index
from   test_mr
where  id <=5;

-- Find the last space
select mrcol, instr(mrcol, ' ', -1) index
from   test_mr
where  id <=5;

-- Starting at the 10th character, find the first space
select mrcol, instr(mrcol, ' ', 10) index
from   test_mr
where  id <=5;


-- Starting at the 1 character, find the third space
select mrcol, instr(mrcol, ' ', 1, 3) index
from   test_mr
where  id <=5;

-- 2. Big values
-- Find the first occurrence of the word 'son'
select mrcol, instr(mrcol, 'son') index
from   test_mr
where  id >5;

-- Starting at the 100th character, find the first occurrence
select mrcol, instr(mrcol, 'son', 100) index
from   test_mr
where  id >5;
...
-- 3. Dynamically generated big values
-- Find the first occurrence of the word 'son'
select mrcol, instr(lower(mrcol), 'son') index
from   test_mr
where  id >5;

-- Starting at the 100th character, find the first occurrence
select mrcol, instr(lower(mrcol), 'son', 100) index
from   test_mr
where  id >5;


-- Starting at the 1st character, find the third occurrence
select mrcol, instr(lower(mrcol), 'son', 1, 3) index
from   test_mr
where  id >5;
