#!/bin/sh
#
# setup.sh: Create a sample database and register idn_fifo.1.1
#
rm -f setup.out
echo " " > setup.out
...
create database fifodb with log;
END_CREATEDB

echo "	Step 3: Register the idn_fifo.1.1 DataBlade" >> setup.out
echo " " >> setup.out
blademgr >> setup.out 2>&1 <<END_BLADEMGR
register idn_fifo.1.1 fifodb
y
quit
END_BLADEMGR
